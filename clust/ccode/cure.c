...
K cure_nn_F(K kpointind,K tree,K clusters,K reps){
  ...
  cure_nnleaf_F(kK(reps)[pointind],children[curr],cluster,&nndist,&nni,kJ(clusters),reps);
  ...
  while(curr){                                             // loop upwards until we've reached the root of the tree
    ...
    if(!v[curr]){                                   // if already visited this node, skip to parent
      if(nndist>kdtree_rdist_F(point,tree,parent)){ // could there be anything closer in the peer branch of the tree?
...
K cure_cluster_dists_F(K cluster1,K cluster2list,K reps){
  F ndist,dist,adist;            // nearest distance, cluster distance, axis aligned distance
...
K cure_nn_E(K kpointind,K tree,K clusters,K reps){
  ...
  cure_nnleaf_E(kK(reps)[pointind],children[curr],cluster,&nndist,&nni,kJ(clusters),reps);
  ...
  while(curr){                                             // loop upwards until we've reached the root of the tree
    ...
    curr=kJ(children[parent])[left[curr]]; // peer
    if(!v[curr]){                                   // if already visited this node, skip to parent
      if(nndist>kdtree_rdist_E(point,tree,parent)){ // could there be anything closer in the peer branch of the tree?
        // jump to leaf in this subtree point would belong to
...
K cure_cluster_dists_E(K cluster1,K cluster2list,K reps){
  E ndist,dist,adist;            // nearest distance, cluster distance, axis aligned distance
  K cluster2;                      // current cluster we're measuring the distance to
  J i,j,k,m,pl=kK(reps)[0]->n;     // some loop counters, and pl the dimensionality of the data

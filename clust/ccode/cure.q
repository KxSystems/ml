...
/\l clust/ccode/kdtree.q

\d .ml
clust.cure.:(::)
clust.cure.i.:(::)
clib:2:[`$path,"/clust/ccode/./cure"]
impf:{clust.cure.i[u]:clib(u:`$"_"sv string(x;y)),z}
...
clust.cure.ccdists:{$["f"=.Q.t type z 0;clust.cure.i.cure_cluster_dists_F;"e"=t;clust.cure.i.cure_cluster_dists_E;'`type][x;y;z]}
/ shrink points towards their mean
clust.cure.shrink:{z-x*z-\:y}
/ x - representative points shrunk by y - compression towards mean from z - a list of points
clust.cure.crep:{t$clust.cure.shrink[y;m]$["f"=t;clust.cure.i.cure_cluster_reps_F;"e"=t;clust.cure.i.cure_cluster_reps_E;'`type][z;x;(t:.Q.t type z 0)$m:avg z]}
/ from pi - a list of indices into reps - a list of representative points, find the closest (other) cluster and distance to it
clust.cure.ccnn:{[pi;tree;clusters;reps]@[;0;clusters]{mi:u?mn:min u:x[;1];(x[;0]mi;mn)}clust.cure.cnn[;tree;clusters;reps]each pi}
/ closest (other) cluster and distance to it from x - a point index (into w - a list of reps) given a tree y and mapping from 
/ w - a list 
clust.cure.cnn:{[x;y;z;w]$["f"=.Q.t type w 0;clust.cure.i.cure_nn_F;"e"=t;i.cure_nn_E;'`type][x;y;z;w]}
clust.cure.imin:{x?min x}


clust.cure.i.cure:{[r;c;n;d]
 oreps:reps:flip d;
 ...
 ndists:flip clust.cure.cnn[;tree;r2c;reps]each r2c; / nearest neighbors and distances to each for each initial representative
 i:0;N:pc-n;   / loop counter, and number of iterations required
 ST1:.z.P;
 while[i<N;
  mci:u,ndists[0;u:clust.cure.imin ndists 1];                               / clusters to merge
  ...
  nreps:clust.cure.crep[r;c;npts];                                          / representatives of new cluster
  reps[nri:count[nreps]#ori]:nreps;                              / overwrite any old representatives with the new ones
  r2l[nri]:nrl:clust.kdtree.searchfrom[tree;;0]each nreps;            / leaf nodes for new representatives
  ...
  cnc:clust.cure.ccnn[nri;tree;r2c;reps];                                   / nearest cluster to the newly merged cluster and the distance to it
  ...
  cdists:@[(pc#0w);u;:;clust.cure.ccdists[nri;;reps]c2r u];                 / calculate cluster distance for each cluster which could be closer
  ...
  ndists[0 1;invalid]:$[count invalid;flip clust.cure.ccnn[;tree;r2c;reps]each c2r invalid;(0#0;0#0f)];  
  ndists[;mci 0]:cnc;
  ndists[;mci 1]:(0N;0w);
  i+:1;
 ];
...
clust.cure.cure:{[r;c;n;d]clust.cure.i.cure[r;c;n;d]`clusters}

clust.cure.cure1:{[r;c;n;d]
 reps:flip d;
 tree:clust.kdtree.create[r;d];
 }

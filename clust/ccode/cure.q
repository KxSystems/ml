...
clust.cure.ccdists:{[x;y;z;df] $["f"=.Q.t type z 0;clust.cure.i.cure_cluster_dists_F;"e"=t;clust.cure.i.cure_cluster_dists_E;'`type][x;y;z;df]}
...
clust.cure.crep:{[x;y;z;df]t$clust.cure.shrink[y;m]$["f"=t;clust.cure.i.cure_cluster_reps_F;"e"=t;clust.cure.i.cure_cluster_reps_E;'`type][z;x;(t:.Q.t type z 0)$m:avg z;df]}
/ from pi - a list of indices into reps - a list of representative points, find the closest (other) cluster and distance to it
clust.cure.ccnn:{[pi;tree;clusters;reps;df]@[;0;clusters]{mi:u?mn:min u:x[;1];(x[;0]mi;mn)}clust.cure.cnn[;tree;clusters;reps;df]each pi}
/ closest (other) cluster and distance to it from x - a point index (into w - a list of reps) given a tree y and mapping from 
/ w - a list 
clust.cure.cnn:{[x;y;z;w;df]$["f"=.Q.t type w 0;clust.cure.i.cure_nn_F;"e"=t;clust.cure.i.cure_nn_E;'`type][x;y;z;w;df]}
clust.cure.imin:{x?min x}
clust.cure.dfd:`e2dist`edist`mdist!1 2 3;



clust.cure.i.cure:{[r;c;n;d;df]
 oreps:reps:flip d;
 ...
 ndists:flip clust.cure.cnn[;tree;r2c;reps;dd:clust.cure.dfd[df]]each r2c; / nearest neighbors and distances to each for each initial representative
 ...
 while[i<N;
  ...
  nreps:clust.cure.crep[r;c;npts;dd];                                          / representatives of new cluster
  ...
  cnc:clust.cure.ccnn[nri;tree;r2c;reps;dd];                                   / nearest cluster to the newly merged cluster and the distance to it
  ...
  cdists:@[(pc#0w);u;:;clust.cure.ccdists[nri;;reps;dd]c2r u];                 / calculate cluster distance for each cluster which could be closer
  ...
  ndists[0 1;invalid]:$[count invalid;flip clust.cure.ccnn[;tree;r2c;reps;dd]each c2r invalid;(0#0;0#0f)];  
  ndists[;mci 0]:cnc;
  ndists[;mci 1]:(0N;0w);
  i+:1;
 ];
...
clust.cure.cure:{[r;c;n;d;df]clust.cure.i.cure[r;c;n;d;df]`clusters}

...
xval.gridsearch:{[x;y;f;p;d]
 d:xval.i.defaults,$[d~(::);();d];
 p!(d[`xval][d`k;d`n][x;y]f pykwargs@)@'p:key[p]!/:1_'(::)cross/value p}
xval.gridsearchfit:{[x;y;f;p;d]
 d:xval.i.defaults,$[d~(::);();d];
 i:(0,floor count[y]*1-d`test)_$[d`shuffle;xval.i.shuffle;til count@]y;
 pr:first key desc avg each xval.gridsearch[x i 0;y i 0;f;p;d];
 (pr;f[pykwargs pr](x;y)@\:/:i)}

xval.fitscore:{[a;p;d].[.[a[][p]`:fit;d 0]`:score;d 1]`}
